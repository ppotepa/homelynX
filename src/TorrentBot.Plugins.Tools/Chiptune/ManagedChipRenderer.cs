...
internal static class ManagedChipRenderer
{
    public static byte[] Render(HardwareSong song)
    {
        ...
        foreach (var note in song.Notes)
        {
            ...
            RenderVoice(song.Chip, note, left, right, start, end, song.SampleRate);
        }
        return Wav(left, right, song.SampleRate);
    }

    private static void RenderVoice(string chip, HardwareNote note, float[] left, float[] right, int start, int end, int rate)
    {
        ...
        for (var i = start; i < end; i++)
        {
            ...
            var sample = Wave(chip, note, phase, ref lfsr, i);
            var gain = (float)(sample * attack * release * decay * note.Velocity / 127d * .16);
            left[i] += gain * (float)(1 - pan); right[i] += gain * (float)(1 + pan);
        }
    }

    private static double Wave(string chip, HardwareNote note, double phase, ref uint lfsr, int sample)
    {
        ...
        return chip switch
        {
            ...
            _ => p < (note.Instrument == "soft_lead" ? .5 : .25) ? 1 : -1
        };
    }
