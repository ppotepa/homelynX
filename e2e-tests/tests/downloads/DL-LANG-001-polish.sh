...
TEST_ID="DL-LANG-001"
TEST_NAME="Polish NL: 'pokaż pobierania'"
...
# Execute
log_step "Sending Polish natural language command"
RESPONSE=$(send_telegram_command "pokaż pobierania")
stop_timer
...
if [[ "$TORRENT_COUNT" -gt 0 ]]; then
    ...
    exit_test "$TEST_ID" "FAIL" "Download info missing"
fi
